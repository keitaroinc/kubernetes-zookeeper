...
def GenerateConfig(context):
  ...
  service = {
      ...
      'properties': {
          ...
          'spec': {
              'ports': GenerateServicePorts(context, 'service_ports'),
                      ...
                      'containers': [
                          {
                              ...
                              'ports': GenerateContainerPorts(context, 'container_ports'),
                          }
                      ]
                  }
              }
...
def GenerateServicePorts(context, name):
  ...
  """
  ports = []
  tmp_ports = context.properties.get(name, [])
  for entry in tmp_ports:
    port = entry.get('port', None)
    target_port = entry.get('target_port', None)
    ...
    if port:
      port_dic['port'] = port
    if target_port:
      port_dic['targetPort'] = target_port
    if protocol:
      port_dic['protocol'] = protocol

    ports.append(port_dic)

  return ports

def GenerateContainerPorts(context, name):
  """Generates a ports section for a containers.

  Args:
    context: Template context, which can contain the following properties:
             port - Port to use for the container
             protocol - Protocol to use.
